  ...
  ({ label, error, className = "", ...props }, ref) => {
    return (
      <div className={cn("flex flex-col", className)}>
        {label && (
          <label className={cn("ml-2 z-10 px-2 text-white bg-perry whitespace-nowrap w-min rounded-t-sm rouded-b-sm text-base font-medium", {
              "bg-red-500": error,
          })}>{label}</label>
        )}
        <input
          ...
          {...props}
        />
        {isString(error?.message) && <span className="mt-1 text-sm text-red-500">{error?.message}</span>}
      </div>
    );
  },
);
